...
(
  ...
  service_category_content text default null,
  ...
  service_category_display_homepage tinyint(1) unsigned default 1 comment 'Hien thi trang chu',
  service_category_created_at datetime not null,
  service_category_updated_at timestamp default CURRENT_TIMESTAMP on update current_timestamp,
  ...
  constraint `fk_service_category_id_locale` foreign key (fk_locale) references locale(locale_id)
) ENGINE=InnoDb AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT 'Danh muc Dich vu tien ich';