...
class Cli_MigrationController extends Application_Controller_Cli
{
     ...
     * @var string
     */
    const SCHEMA_UPDATE_TABLE = 'schema_updated';
    const SCHEMA_UPDATE_FILENAME = '_init.00.sql';
    ...
    /**
     * Check if schema_updated table exists
     * @return bool
     */
    private function _isSchemaExsist()
    {
        $checkSchemaQuery = sprintf(
            'SHOW TABLES LIKE %s',
            $this->_db->quote(self::SCHEMA_UPDATE_TABLE)
        );


        return $this->_db->query($checkSchemaQuery)->rowCount() > 0;
    }

    /**
     * Initiate schema
     */
    private function _initSchema()
    {
        $schemaInitFile = $this->_path_script . '/' . self::SCHEMA_UPDATE_FILENAME;


        if (!file_exists($schemaInitFile)) {
            echo '!!! Schema Updated file does not exist in: ' . $schemaInitFile . PHP_EOL;
            die();
        }

        $content    = trim(file_get_contents($schemaInitFile));
        ...
        try {
            ...
            foreach ($statements as $statement) {
                ...
                if ($statement) {
                    echo '-------------- START SCHEMA INITIATION' . PHP_EOL;
                    echo 'Running:' . PHP_EOL . $statement . PHP_EOL . PHP_EOL;
                    $this->_db->query($statement);
                }
            }

            echo '-------------- FINISHED SCHEMA INITIATION' . PHP_EOL . PHP_EOL;

            $this->_db->commit();
        } catch (Exception $e) {
            $this->_db->rollBack();
            echo PHP_EOL;
            echo '!!! FAILED to initiate SCHEMA UPDATED !!!' . PHP_EOL . PHP_EOL;
            echo 'Exception message: ' . $e->getMessage() . PHP_EOL . PHP_EOL;
