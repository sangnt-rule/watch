...
class Admin_Model_Admin extends Application_Singleton
{
    ...
    public function updateLastLogin($idAdmin)
    {
        ...
        try {
            ...
            $data = array(
                DbTable_Admin::COL_ADMIN_LAST_LOGIN => $this->_dao->mysqlSysDate(),
                DbTable_Admin::COL_ADMIN_LAST_LOGIN_IP => $ipAddress
            );
            $where = sprintf('%s=%d', DbTable_Admin::COL_ADMIN_ID, $idAdmin);
    ...
    public function manualUpdateActive($value, $id)
    {
        ...
        $params = array(DbTable_Admin::COL_FK_CONFIG_ACTIVE => intval($value));
    ...
    public function getAllPrivilegesByFkAdminRole($fk_role)
    {
        ...
        if ($privilegeData) {
            $result = $this->formatPrivilegeData($privilegeData);
        }
        return $result;
    }
     ...
     * @param Zend_Db_Table_Rowset_Abstract $privilegeData
     * @return array
     */
    public function formatPrivilegeData($privilegeData)
    {
        $result = array();
        foreach ($privilegeData as $data) {
            $component = Admin_Model_AdminComponent::getInstance()->getNameById($data->{DbTable_Admin_Module::COL_FK_ADMIN_COMPONENT});
            if (!isset($result[$component])) {
                $result[$component] = array();
            }
            $module = $data->{DbTable_Admin_Module::COL_ADMIN_MODULE_NAME};
            if (!isset($result[$component][$module])) {
                $result[$component][$module] = array();
            }
            $resource = $data->{DbTable_Admin_Resource::COL_ADMIN_RESOURCE_NAME};
            if (!isset($result[$component][$module][$resource])) {
                $result[$component][$module][$resource] = array();
            }
            $result[$component][$module][$resource][] = array(
